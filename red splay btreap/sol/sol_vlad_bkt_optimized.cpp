...
vector<int> find_not_fixed_nodes(const Graph& T) {
  vector<int> answer;
  int N = T.size();
  for (int i = 1; i < N; i++) {
    if (T[i].size() == 2) {
...
int count_solutions(const Graph& T) {
  ...
  auto not_fixed_nodes = find_not_fixed_nodes(T);
  int M = not_fixed_nodes.size();
  for (int mask = 0; mask < (1 << M); mask++) {
    vector<Color> colors(N, BLACK);
    for (int i = 0; i < M; i++) {
      Color bit = (mask & (1 << i)) ? RED : BLACK;
      colors[not_fixed_nodes[i]] = bit;
    }
    answer += check_solution(T, colors, false);
  }
  return answer;
}
